...
app.put('/api/v1/storage', login.checkAuth, function (req, res) {
    ...
    if (req.body.check !== "true") {
        fs.readFile(req.files.file.path, 'utf8', function (err, fileData) {
            ...
            fhir.createDocumentReference(req.user.username, req.files.file.name, req.files.file.type, fileData, function (err, resp, body) {
                ...
                if (err) {
                    ...
                    importRecord(req.user.username, req.files.file, fileData, function (err, import_results) {
                        if (err) {
                            ...
                            res.status(200).end();
                            record.saveEvent('fileUploaded', req.user.username, "User uploaded '" + import_results.name, import_results.name, function (err) {
                                if (err) {
                                    res.status(400).send('Event error ' + err);
                                } else {
                                    res.status(200).end();
