...
var debug = require('debug')('dre.services.storage');
...
function parseRecord(record_type, record_data, callback) {
    ...
    if (record_type === 'application/xml' || record_type === 'text/xml' || record_type === 'text/plain' || record_type === 'application/json') {
        extractRecord(record_data, function (err, format_type, parsed_record) {
            if (err) {
                console.log("parse record error");
                callback(err);
            } else {
...
function importRecord(username, record_metadata, record_data, callback) {
    record.saveSource(username, record_data, record_metadata, record_metadata.type, function (err, fileInfo) {
        if (err) {
            callback(err);
        } else {
...
function importHL7Record(username, record_metadata, record_data, parsed_record, callback) {
    ...
    record.saveSource(username, record_data, record_metadata, "hl7", function (err, id) {
        if (err) {
            ...
            callback(null, id); //from reconcile Recordbelow
...
//Returns binary resource from storage.
app.get('/api/v1/storage/resource/:resourceType/:resourceId/content', login.checkAuth, function (req, res) {
    // console.log(login.checkAuth);
    fhir.getResource(req.params.resourceType, req.params.resourceId, function (err, body, statusCode) {
        if (err) {
            res.status((statusCode) ? statusCode : 400).send(err.message);
        } else {
            var data = JSON.parse(body);
            if (data.content) {
                var tmp = data.content;
                console.log(tmp);
                console.log(new Buffer(tmp, 'base64').toString('utf8'));
                res.setHeader('Content-Type', data.contentType);
                var filename = (_.contains(data.contentType, 'application/xml')) ? 'refine.xml' : 'refine.txt';
                res.setHeader('Content-Disposition', 'attachment; filename=' + filename);
                res.status(200).send(new Buffer(data.content, 'base64').toString('ascii'));
            } else {
...
//Returns resource from storage. (aka proxy to FHIR server)
app.get('/api/v1/storage/resource/:resourceType/:resourceId', login.checkAuth, function (req, res) {
    // console.log(login.checkAuth);
    fhir.getResource(req.params.resourceType, req.params.resourceId, function (err, data, statusCode) {
        if (err) {
            res.status((statusCode) ? statusCode : 400).send(err.message);
        } else {
...
app.get('/api/v1/storage/fhir/*', passport.authenticate('bearer', {
    session: false
}), login.checkAuth, function (req, res) {
    var url = req.originalUrl.substr('/api/v1/storage/fhir/'.length);

    if (url === 'Patient') {
        url = 'Patient?_id=' + req.user.username;
    }
    debug('fhir request url %s', url);

    var whitelist = ['MedicationOrder?patient=' + req.user.username, 'Patient?_id=' + req.user.username];

    var inWhiteList = _.any(whitelist, function (value) {
        return _.startsWith(url, value);
    });

    debug('fhir request any %s %j', inWhiteList, whitelist);

    if (inWhiteList) {
        fhir.proxy(url, function (err, data, statusCode) {
            if (err) {
                res.status((statusCode) ? statusCode : 400).send(err.message);
            } else {
                res.setHeader('Content-Type', 'application/json+fhir');
        ...
        res.status(403).end('Forbidden ' + req.originalUrl);
    }
});
...
app.put('/api/v1/storage', login.checkAuth, function (req, res) {
    ...
    //console.log('headersSent ', res.headersSent, req.files.file.path)
    ...
    if (req.body.check !== "true") {
        fs.readFile(req.files.file.path, 'utf8', function (err, fileData) {
            if (err) {
                ...
                fhir.createDocumentReference(req.user.username, req.files.file.name, req.files.file.type, fileData, function (err, docRefLocation) {
                    if (err) {
                        debug(err);
                        return res.status(400).send(err);
                    }
                    fhir.parseAndSave(req.user.username, req.files.file.name, req.files.file.type, fileData, docRefLocation, function (err, links) {
                        if (err) {
                            debug(err);
                            return res.status(400).send(err);
                        } else {
                            fhir.createProvenanceRecord(req.user.username, docRefLocation, links, function (err, body) {
                                if (err) {
                                    debug(err);
                                    return res.status(400).send(err);
                                } else {
                                    return res.status(200).send(JSON.stringify(links));
                                }
                            });
                        }
                    });
