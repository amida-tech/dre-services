...
app.put('/api/v1/storage', login.checkAuth, function (req, res) {
    ...
    if (req.body.check === "true" && req.files.file.type !== 'application/pdf') {
        ...
        if (req.files.file.type === 'application/pdf') {
            fs.readFile(req.files.file.path, 'utf8', function (err, fileData) {
                if (err) {
                    ...
                    importRecord(req.user.username, req.files.file, fileData, function (err, import_results) {
                        if (err) {
                            console.log('import error: ' + err);
                            res.status(400).send('import error: ' + err);
                        } else {
                            console.log('fileUploaded');
                            res.status(200).end();
                            /*
                             record.saveEvent('fileUploaded', req.user.username, "User uploaded '" + record_data.name, record_data.name, function (err) {
                             if (err) {
                             res.status(400).send('Event error ' + err);
                             } else {
                             res.status(200).end();
...
app.put('/api/v1/storage/extension', login.passport.authenticate('local'), function (req, res) {
    ...
    var file = req.files[fileNames[0]];
    file.name = fileNames[0];
    file.originalFilename = fileNames[0];
    ...
    if (file.type === 'application/pdf') {
        fs.readFile(file.path, 'utf8', function (err, fileData) {
            if (err) {
                ...
                importRecord(req.user.username, file, fileData, function (err, import_results) {
                    if (err) {
                        console.log('import error: ' + err);
                        res.status(400).send('import error: ' + err);
                    } else {
        ...
        processUpload(req.user.username, file, function (err) {
            if (err) {
                res.status(400).send(err);
            } else {
                console.log('fileUploaded');
                res.status(200).end();
                /*
                record.saveEvent('fileUploaded', req.user.username, "User uploaded '" + file.name, file.name, function (err) {
                    if (err) {
                        res.status(400).send('Event error ' + err);
                    } else {
                        res.status(200).end();
