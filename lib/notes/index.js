...
function addNote(ptKey, entry, section, note, star, callback) {
    record.addNote(ptKey, section, entry, note, star, function (err, result) {
        if (err) {
            callback(err);
        } else {
            callback(null, result);
...
app.post('/api/v1/notes/add', login.checkAuth, function (req, res) {
    addNote(req.user.username, req.body.entry, req.body.section, req.body.note, req.body.star, function (err, result) {
        if (err) {
            console.log("err: " + err);
            res.status(400).send(err);
        } else {
