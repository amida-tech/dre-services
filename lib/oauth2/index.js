...
var debug = require('debug')('dre-oauth');

var databaseServer = process.env.MONGO_PORT_27017_TCP_ADDR || 'localhost';
...
var connection = mongoose.createConnection('mongodb://' + databaseServer + ':27017/dre');
...
var clients = module.exports.clients = (function () {
    ...
    var testClients = [{
        ...
        /*{
                name: 'Argonaut Demo Client (Medications List)',
                clientId: 'argonaut_demo_client_amazon',
                clientSecret: 'have no secrets!',
                launchUri: '',
                redirectUri: '',
                logoUri: '',
            ...
            }, */
        {
            ...
            name: 'Argonaut Demo Client (AWS)',
            clientId: 'argonaut_demo_client_amazon',
            clientSecret: 'have no secrets!',
            launchUri: '',
            redirectUri: 'http://argonaut.amida-demo.com:3001/fhir/callback',
            logoUri: '',
            tokenEndpointAuthMethod: 'none'
        }
    ];
...
module.exports.authorization = [
    ...
    function (req, res) {
        ...
        /*var html = '<DOCTYPE html><html><head><style>body{background: rgb(75,70,83);}</style><head><body></body>' +
            '<p>Hi <%= user.name %>!</p>' +
        ...
        var html = "";
        var loginPage = path.join(__dirname, 'oauth-authorize.html');
...
module.exports.displaylogin = function (req, res) {
    var html = "";
    var loginPage = path.join(__dirname, 'oauth-login.html');
    fs.readFile(loginPage, {
        encoding: "utf8"
    }, function (err, data) {
        if (err) {
            ...
            html = html.replace('<%= loginUri %>', req.originalUrl)
                .replace('<%= msg %>', ((req.query.msg) ? '<h3><span class="label label-warning">' + req.query.msg + '</span></h3>' : ''))
                .replace('<%= clientId %>', req.query.clientId || req.body.clientId)
                .replace('<%= redirectUri %>', req.query.redirectUri || req.body.redirectUri)
                .replace('<%= responseType %>', req.query.responseType || req.body.responseType);
            debug(html);
            res.write(html);
            res.end();
        }
...
module.exports.login = [
    ...
    function (req, res, next) {
        passport.authenticate('local', function (err, user, info) {
            ...
            if (!user) {
                var redirect = '/oauth2/login';
                var q = [];
                if (req.query.clientId) {
                    q.push('client_id=' + req.query.clientId);
                }
                if (req.query.redirectUri) {
                    q.push('redirectUri=' + encodeURIComponent(req.query.redirectUri));
                }
                if (req.query.responseType) {
                    q.push('response_type=' + req.query.responseType);
                }
                if (req.body.username) {
                    q.push('msg=Wrong%20username%20or%20password');
                }
                if (q.length > 0) {
                    redirect += '?' + q.join('&');
                }
...
module.exports.populate = function (callback) {
    clients.populate(function (err) {
        if (err) {
            callback(err);
        } else {
            callback(null);
