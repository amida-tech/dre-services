...
var parseAndSave = function (patientId, filename, contentType, stringBlob, docRefLocation, cb) {
    ...
    var post2fhir = function (data, parserName, docRef) {
        ...
        async.waterfall([
            /** Pull Patient record */
            function (cb) {
                request({ method: 'GET', uri: fhirServerUrl + 'Patient/' + patientId },
                    function (err, request, body) { cb(err, request, body); });
            },
            ...
            function (request, body, cb) {
                ...
                //debug("body %s", body);
                //debug("cb ", cb);
                var idx = _.findIndex(data.entry, function (value) { return value.resource.resourceType === 'Patient'; });
                if (idx !== -1) {
