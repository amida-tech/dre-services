...
var updateProfile = function (username, profile, cb) {
    ...
    var opts = {
        method: 'GET',
        uri: fhirServerUrl + 'baseDstu2/Patient/' + username
    };
...
var createPatient = function (account, cb) {
    ...
    var opts = {
        method: 'PUT',
        uri: fhirServerUrl + 'baseDstu2/Patient/' + account.username,
        json: true,
        body: patient
    };
...
var createDocumentReference = function (patientId, filename, contentType, stringBlob, cb) {
    ...
    var opts = {
        method: 'POST',
        uri: fhirServerUrl + 'baseDstu2/Binary',
        json: true,
        body: binary
    };
    ...
    request(opts, function (err, resp, body) {
        if (!(body.issue && _.any(body.issue, function (value) {
            ...
            var opts = {
                method: 'POST',
                uri: fhirServerUrl + 'baseDstu2/DocumentReference',
                json: true,
...
var parseAndSave = function (patientId, filename, contentType, stringBlob, cb) {
    ...
    var post2fhir = function (data, parserName) {
        ...
        request(opts, function (err, resp, body) {
            ...
            if (cb) {
                return cb(err, _.map(
                    _.filter(body.entry, function (entry) {
                        return entry.hasOwnProperty('transactionResponse');
                    }),
                    function (entry) {
                        return entry.transactionResponse.location;
                    }));
            }
        });
    };
    ...
    if ((contentType === 'application/xml' || _.endsWith(filename, '.xml'))) {
        ...
        istream.pipe(new c32_parser.C32ParserStream()).on('data', function (data) {
            ...
            if (_.isError(data)) {
                istream.pipe(new cda_parser.CcdaParserStream(patientId)).on('data', function (data) {
                    debug("CCDA parser done (error: %j)", _.isError(data));
                    if (_.isError(data)) {
                        istream.pipe(new ccd_parser.CcdParserStream()).on('data', function (data) {
...
var createProvenanceRecord = function (patientId, docRefLocation, links, cb) {
    ...
    if (_.startsWith(docRefLocation, fhirServerUrl + 'baseDstu2/')) {
        docRefLocation = docRefLocation.substring((fhirServerUrl + 'baseDstu2/').length);
    }
    ...
    var post2fhir = function (data) {
        var opts = {
            method: 'POST',
            uri: fhirServerUrl + 'baseDstu2' + '/Provenance',
            json: true,
            body: data
        };
