...
function newAccount(inputBody, done) {
    ...
    User.register(account, inputPassword, function (err, user) {
        if (err) {
            ...
            fhir.updatePatient(inputBody, function (err, resp, body) {
                done(err, user);
            });
        }
    });
...
app.post('/api/v1/updateprofile', checkAuth, function (req, res) {
    saveRegProfile(req.body, function(err, input) {
        input.username = req.user.username;
        fhir.updatePatient(input, function (err, resp, body) {
            record.saveEvent('updateAccount', req.user.username, "User updated account '" + req.user.username + "'", null, function (err) {
                if (err) {
                    res.status(400).send('Event error ' + err);
                } else {
                    res.status(200).end();
